...
def isEven(n):
    ...
    Недостатки:
      - скорость работы зависит от порядка числа: большие числа обрабатываются медленее;
...
def isEvenBitwise(n):
    ...
    Описание: воспользуемся тернарным оператором, показанном в методе isEvenRefactored, но при
    ...
    Преимущества:
      ...
      - скорость работы не зависит от того, большое число сравнивается или нет.
